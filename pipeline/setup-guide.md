...
| 汇总说「本次评审的提交…已不是 Codeup 侧最新的合并源版本」且没有行内评论 | 正常：评审期间源分支又有新推送，本次评的是旧提交，行号绑到新版本上会挂错位置，所以脚本一条行内评论都不发（fail-closed），问题以完整清单形式留在汇总里。新推送触发的那次评审会补上行内评论。若这条提示反复出现，查是否有自动化（格式化机器人、自动 rebase 等）在持续往源分支推送 |
...
5. **版本对**：日志里「行内评论版本对：from=… → to=…」缺失，说明版本列表查不到或选不出
   ...
   并在评论里写明原因。
6. **版本与 HEAD 对不上**：日志出现「最新合并源版本的提交…与当前 HEAD…不一致」= 评审期间又推了一次，
   本次**一条行内评论都不发**（fail-closed：行号是按旧提交算的，绑到新版本上会挂错位置），汇总退回完整
   问题清单并写明「已不是 Codeup 侧最新的合并源版本」；新推送触发的那次评审会补上。
...
- 最新合并源版本的提交不是本次评审的提交（评审期间源分支又推送了）时同样退回完整清单、一条行内评论都不发：
  行号是按本次评审的提交算的，绑到新版本上会挂错位置；新推送触发的那次评审会把行内评论放对。
- 评审员给出「可合并」却报了 P0 时，结论按「不建议合并」渲染并在结论行下明说原因；结论不是三个契约取值之一时
  显示「评审员未给出契约内的结论」，原值只进流水线日志。同一轮里逐字段相同（文件、行区间、级别、标题）的
  问题只保留首条，日志记「N 条完全重复的问题已合并」。
