...
| 汇总说「本次评审的提交…与 Codeup 侧最新合并源版本…不一致」且没有行内评论 | 预期行为：行号是按本次评审的提交算的，绑到另一个版本上会挂错位置，所以脚本一条行内评论都不发（fail-closed），问题以完整清单形式留在汇总里。两种成因，脚本不区分：① 评审期间源分支又推送了（本次评的是旧提交）——新推送触发的那次评审会补上行内评论，反复出现就查是否有自动化（格式化机器人、自动 rebase 等）在持续推送；② Codeup 的版本列表还没为本次推送建出版本（本次评的反而是新提交）——这种不会自愈，若同一个提交连续多次都这样，在 MR 页面确认「版本」列表里有没有该提交，必要时向源分支再推一次触发建版本 |
| 汇总说「Codeup 版本列表未给出该版本的提交号」且没有行内评论 | 同样是 fail-closed：接口返回的最新合并源版本里没有 `commitId`，脚本无从证明行内评论会绑到本次评审的提交上（I5），于是退回完整清单。多为接口返回字段缺失或改名，查日志里那条警告与 `GET …/diffs/patches` 的返回；`INLINE_COMMENT=0` 的完整清单形态不受影响 |
...
5. **版本对**：日志里「行内评论版本对：from=… → to=…」缺失，说明版本列表查不到或选不出
   ...
   并在评论里写明原因。
6. **版本与 HEAD 对不上**：日志出现「最新合并源版本的提交…与当前 HEAD…不一致」，本次**一条行内评论都不发**
   （fail-closed：行号是按本次评审的提交算的，绑到另一个版本上会挂错位置），汇总退回完整问题清单并写明
   「与 Codeup 侧最新合并源版本…不一致」。成因两种、脚本不区分：评审期间又推了一次（下一轮评审会补上），
   或 Codeup 的版本列表还没为本次推送建出版本（不会自愈）。日志里「没有提交号」= 接口没返回 `commitId`，
   同样 fail-closed（绑不上就不发）。
   「最新合并目标版本的提交…不等于本地 merge-base」= 目标分支在 MR 分出后前进过，这条**照发**（行号是
   新文件侧的，比较基准不同不改变行号），只在汇总评论里带一句「行号可能有偏移」的提示。
...
- 最新合并源版本的提交与本次评审的提交不一致、或接口没给出该版本的提交号时，同样退回完整清单、一条行内评论
  都不发：行号是按本次评审的提交算的，绑到另一个版本上会挂错位置，绑不上更不能猜（I5）。前者若是评审期间的
  新推送，新推送触发的那次评审会把行内评论放对。
- 评审员给出「可合并」却报了 P0 时，结论按「不建议合并」渲染并在结论行下明说原因；结论不是三个契约取值之一时
  显示「评审员未给出契约内的结论」，原值只进流水线日志。同一轮里逐字段相同（文件、行区间、级别、标题）的
  问题只保留首条，日志记「N 条完全重复的问题已合并」；没有文件位置的仓库级问题另加正文原文比对，措辞不同的
  两条不会被并掉。
