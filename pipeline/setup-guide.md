...
5. **必做：给评审任务配流水线任务超时**（任务设置里的超时，建议 `KIRO_TIMEOUT` + 10 分钟，默认值下即 25 分钟）。
   ...
   脚本自己的硬上限（文件数 / 原始字节 / 单 chunk / 预处理墙钟 + 可行动的失败评论）是 Phase 2 backlog（issue 19）。
...
| `DIFF_SIZE_LIMIT` | `307200` | diff ≤ 300KB 时全量直传；超出按优先级取舍，省略文件落盘为 diff 片段并给评审员索引 | 字节数（**纯数字**，不支持 `300KB`）；调低可省 credit，调高会增加单次成本与超时风险。**它只是提示词预算**：不限制原始 diff 总量、磁盘、变更文件数与预处理时间（超限时脚本仍把整份 diff 落盘、逐文件生成 chunk），`KIRO_TIMEOUT` 也不包住这段预处理；并发 = 1 下一个极端 MR 会占住整条流水线，务必给流水线任务配超时（第 4 节第 5 步）。硬上限是 Phase 2 backlog（issue 19） | 非 ≥1 整数时**拒绝运行**并回写「评审未完成」（否则整份 diff 都进省略清单，评审员只拿到一份索引） | Flow |
