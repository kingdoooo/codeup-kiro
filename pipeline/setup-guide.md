...
2. 它绑定的 RAM 身份只需要**该专用桶的只读**权限,但要**两条**——只给 `oss:GetObject` 装不上:
   ...
   ✅ 已实测(2026-09-18,专门建一个只挂上面这份策略的临时 RAM 用户,真 ossutil v1.7.18,桶 `ACL=private`
   且无 bucket policy——排除桶级放权干扰):

   | 身份 | 列举前缀 | 递归下载 `cp -r` |
   |---|---|---|
   | 只挂上面这两条 | 通过 | 通过 |
   | **只挂 `oss:GetObject`** | **拒绝** | **拒绝** |
   | 零权限(对照组) | 拒绝 | 拒绝 |

   两个方向都成立:这份策略**够用**,而 `oss:ListObjects` 那条**确实必需**——只给 `GetObject` 时连列举都过不去,
   递归下载在第一步就被拒。零权限对照组被拒是测试有效性的前提(三个身份结果不同,才说明受限凭证真的生效了)。
   证据分级:探测用的 ossutil 是 **1.7.18**,而流水线里 `OSSDownload` 实测是 **1.7.19**,差一个补丁版本;
   「授权是否充分」验到「列举通过 + 下载已开始传输」为止,最小权限身份下没有把 586 MB 跑完
   (完整字节数与摘要由第 9 节的备包核对和 2026-09-16 的真实流水线运行覆盖,那两次用的是管理员权限身份)。
...
3. **PATH 必须是一组「绝对、且不在业务仓库内」的目录**（脚本第 0 步会检查，不合格就拒绝运行）。
   ...
   ⚠ **这道门管不到 `review_step` 的 run 块本身**：run 块整段跑在 Flow 的**外层** shell 里，比脚本第 0 步更早，
   而任务 cwd 就是业务库。所以 run 块里**任何裸外部命令都会按 PATH 查找** = 让 MR 作者在所有安全检查之前
   提供那个命令。参考 YAML 因此把搬包的三个命令写成 `/bin/mkdir`、`/bin/mv`、`/bin/ls`（`[`/`echo`/`export`/`:`
   是 builtin，不走 PATH）；你自己往 run 块里加命令时也要照这个口径。
   ✅ 已实测（2026-09-18）：业务库提交 `bin/{mkdir,mv,ls}` 三个 canary、执行器 PATH 注入业务库目录后——
   修复版 canary **零调用**且搬包真的完成（目标文件在、源文件消失、内容正确）；同一条件下把三个命令换回裸写法，
   canary **确实被调用**并拦在真 `mv` 之前。两个方向都成立，判据不是空转。
...
13. 模型是否生效：agent 配置指定了 `"model": "gpt-5.6-sol"`（GPT-5.6 Sol，Kiro 官方已上线，实验性支持，
    ...
    **脚本已自动核对（issue 16，第 3.1 步）**：每次评审在启动 Kiro 之前跑一次 `chat --list-models`
    （零成本，与 `chat --help` / `--version` 同一批），把 agent 定义里的 `model` 值与清单比对——
    - 在清单内 → 流水线日志一行「模型核对通过」，评论上什么都不加；
    - **不在清单内 → 汇总评论挂一条 notice**（写出具体 ID，说明 Kiro 会静默回落默认模型、本次评审仍已完成），
      **但不拒绝评审**：模型换了评审仍然有价值，拒绝是过度反应；
    - agent 定义里没有 `model` 字段 → 跳过核对（那是刻意用默认模型），连 `--list-models` 都不调；
    - **核对自身失败一律 fail-open**：命令失败、超时、或将来 kiro-cli 改了输出格式导致解析不出清单，
      都只写一行流水线日志、不挂 notice、不阻断评审。一个建议性检查的解析失败去拦评审，方向是反的。

    所以你要做的只有：看到那条 notice 时，用 `kiro-cli chat --list-models` 对一遍 agent 里的 model 值
    （ID 轮换或组织策略变更都会让它不在清单内）。另有一条事后旁证：流水线日志里的「Kiro 用量：credits=…」，
    倍率差异是唯一可观测的信号——评审用量突然掉到原来的几分之一，就要怀疑模型回落了。
