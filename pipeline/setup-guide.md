...
5. **验证**：`GetUserByToken`（`GET /oapi/v1/platform/user`，带该令牌）确认 name=`Kiro`；
   在目标库发一条测试评论确认权限，作者与头像应显示为 Kiro。
...
3. 重跑流水线：确认 Kiro 汇总评论仍只有一条、内容被**原地更新**且计数 +1
   （页脚「第 2 次评审」、「历次评审」表两行），评论 ID 不变。
...
| 汇总评论每次都新建一条 | 未配置 CODEUP_BOT_USERNAME（日志会打出新建评论的作者用户名，照抄配上即可） |
| 开了 INLINE_COMMENT=1 但没有行内评论 | 看日志：①「行内评论未发出」→ 版本列表查不到或选不出版本对，此时汇总退回完整清单；② 问题都在「未定位问题」小节 → 模型给的行号不在本次变更行集合内 |
| 重跑后同一行出现重复的行内评论 | 去重指纹是「文件 + 行 + 标题」：标题或行号变了就是新的一条。其余成因看日志：「本次跳过去重」= 查现有行内评论失败；上一次的评论已被删除；或那条评论已 out_dated（绑在被取代的旧版本上，会按当前版本重发一条） |
| 报错「需要 sha1sum 或 shasum」 | INLINE_COMMENT=1 依赖它算去重指纹；构建机安装 coreutils 或 perl |

## 11. 行内评论开关（INLINE_COMMENT）
默认全部关闭，观感与之前完全一致。要启用行内评论，在「变量和缓存」加：

| 变量 | 默认 | 取值 | 说明 |
|---|---|---|---|
| `INLINE_COMMENT` | `0` | `0` / `1` | `1` = 可定位的问题发成行内评论，汇总退化为状态面板 + 折叠区；其它取值直接拒绝运行 |
| `INLINE_PROFILE` | `quiet` | `quiet` / `balanced` / `critical` | 哪些级别可发行内：quiet=P0+P1、balanced=全部、critical=仅 P0。非法取值回落 quiet 并打警告 |
| `MAX_INLINE_COMMENTS` | `10` | 非负整数 | 单次上限，超出的进折叠区。非法取值回落 10 并打警告 |
| `CODEUP_BOT_USERNAME` | 空 | 形如 `aliyun:xxx` | 机器人账号用户名。汇总评论原地更新与行内评论去重都靠它。首次运行后在日志里找「新建评论的作者用户名=…」照抄 |

开启后的行为：
- 只有**新文件侧**、且落在本次变更行集合内的行才会收到行内评论（Codeup 只支持新文件侧行号）。
  行号不在集合内、或问题没给文件/行号的，进汇总评论的折叠区「未定位问题」。
- 行内评论以草稿创建、一次性提交，**不带评审意见**（不参与合并卡点）。
- 重跑不会重复：发布前按「文件 + 行 + 标题」的指纹跳过本账号已发过的同一条。
- 版本列表查不到或选不出版本对时，自动退回「一条含完整问题清单的汇总评论」，并在评论里说明原因。
