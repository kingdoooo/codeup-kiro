...
| 评论里的章节标题显示为普通文字 | Codeup 评论的 Markdown 只渲染 `#` 与 `##` 两级标题，`###` 及以下按普通段落显示（2026-09-04 实测；加粗、表格、折叠块、代码块、引用、分隔线都正常）。模板已按 `#`（评论标题）/ `##`（章节）/ 加粗（问题分组、每条问题、折叠区小节、行内评论首行）设计；看到章节平铺成小字，说明用的是 2026-09-04 之前的集成包版本，更新即可。自行改渲染模板时不要用三级以下标题 |
...
| `CODEUP_COMMENT_PAGE_HINT` | `100` | 评论列表返回条数达到该值时打「已达常见单页上限」告警（评论接口未做分页） | 调高会让那条告警不再出现，于是「旧汇总落在页外→误判首次评审」失去提示 |
