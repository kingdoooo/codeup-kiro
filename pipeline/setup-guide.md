...
| `KIRO_PINNED_ARTIFACT` | 空 | `latest` 档位下忽略 | `pinned` 档位**必填**：安装包在执行器上的**绝对路径**，由流水线的 OSS 下载步骤（或制品源等价机制）预先备好。评审脚本不知道文件是怎么到那儿的，所以自建执行器把 kiro-cli 烤进镜像时同一套脚本不用改 | 必须是绝对路径，且**不得落在业务仓库之内**——业务库内容一律不受信，否则业务库能自己放一个「安装包」；摘要门能挡内容替换，但路径判据独立成立、不靠摘要兜底。取值不回显 | Flow |
