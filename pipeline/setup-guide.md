...
| 汇总说「Codeup 版本列表尚未包含本次提交（滞后）」且没有行内评论 | 同为 fail-closed，成因相反：那个版本是本次提交的**祖先**，说明 Codeup 还没为本次推送建出版本（流水线被触发得比建版本更快）。脚本已按退避重查 3 次仍未等到（日志里有「按退避重查至多 3 次」与每次的结果），这一种**不会自愈**——重跑流水线即可；若同一个提交连续多次都这样，在 MR 页面确认「版本」列表里有没有该提交 |
...
6. **版本与 HEAD 对不上**：脚本先把 Codeup 给的提交号在本地克隆里规范化（`git rev-parse`，所以 12 位缩写与
   ...
   新推送（下一轮评审会补上）；「是当前 HEAD 的祖先」= 版本列表滞后，已按退避重查至多 3 次，仍滞后则退回完整
   清单（重跑流水线即可）；「分属两条历史」= 源分支被改写或强推。另有「没有提交号」= 接口没返回 `commitId`
   （或只有空白），同样 fail-closed（绑不上就不发）。
...
- 行内评论只绑定它所评审的那个提交（ADR-0005）：Codeup 侧最新合并源版本的提交号先在本地克隆里规范化，
  与本次评审的提交不一致、或接口没给出提交号时，一条行内评论都不发、退回完整清单。三种成因分别写进汇总：
  新推送（下一轮评审会补上）、版本列表滞后（已退避重查 3 次，重跑流水线即可）、历史被改写。
