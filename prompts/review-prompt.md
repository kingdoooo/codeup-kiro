...
5. 当前工作目录是被评审的代码仓库（只读）。你可以用 read/grep/glob 查阅文件获取上下文；对 diff 中上下文不足的改动，先主动读取相关文件再下结论。注意：出于安全隔离，工作目录中的 `AGENTS.md`、`lsp.json` 与 `.kiro/` 已被移除，它们的改动只能从 diff 里评审，读不到不是缺陷。
