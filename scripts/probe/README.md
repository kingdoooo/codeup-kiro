...
**提示词的传法：探测与生产都走 stdin、都不给位置参数**（issue 13，2026-09-18；**这条推翻了票 18 ⑬ 的旧决策**）。

kiro-cli 2.21.1 的 `chat [INPUT]` 一旦收到位置参数就**整个忽略 stdin**（本机 env -i / 完整环境 × 空 cwd /
业务库 cwd 四种组合一致，见 setup-guide §8 第 14 项与 ADR-0004 的实测备注）。生产（`scripts/kiro-review.sh`）
自 `06d5028` 起把**运行时提示词与评审输入拼成一份 stdin、不给位置参数**——否则模型收不到 diff，只会自己去读
工作树（那正是 D4 的阻断项）。

旧决策让探测继续用位置参数，理由是「探测的提示词都短、不喂 diff，位置参数最直白，保持一次调用只测一件事的
最小形态」。**推翻它的理由**：上面那条实测事实同时说明，位置参数与 stdin 在 kiro-cli 内部**不是同一条输入
路径**；而已探测名单（`KIRO_TESTED_TARGETS`）的全部价值来自「这个平台+版本我探测过」。探测跑的是生产不用的
那条路径时，名单就是在为一个不用的形状背书——这是**验收依据本身不成立**，比「最小形态」更重要。改成 stdin
并不破坏最小形态：喂进去的仍然只有那一条短提示词，没有 diff。

现在两边都是 stdin，所以**改任何一边都不要再引入位置参数**。刻意仍与生产不同的只有 `--trust-*`：
T5/T6 传 `--trust-tools`、T7 传 `--trust-all-tools`，那是正控与 INFO 用例要测的东西（生产绝不传，端到端
断言参数里没有任何 `--trust-*`）。每个用例喂进去的 stdin 落盘成 `${PROBE_KEEP_DIR}/<用例名>.stdin`，可直接核对。
