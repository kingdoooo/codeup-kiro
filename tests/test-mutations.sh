...
# ============ 票 16 / 16-fix2 的守卫（方案 C：字段级掩码 + 严格保行的文档级兜底）============
# 端到端 A10（tests/test-kiro-review.sh）断言汇总正文、行内正文、流水线日志三处都不含 token 原文。下面的变异各自只杀掉一处，
...
pkg=$(make_mutant m-g-log-redact 's|^  log "错误：\$(_redact_for_log "\$\*")"$|  log "错误：$*"  # 变异 M-g：日志行不掩码|; s|runFinished.status=\$(_redact_for_log "\$(tr -d .\\n. < "\$WORK/contract.json")")|runFinished.status=$(tr -d "\\n" < "$WORK/contract.json")|')
